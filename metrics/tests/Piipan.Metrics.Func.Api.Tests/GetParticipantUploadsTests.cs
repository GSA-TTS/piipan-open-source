using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Logging;
using Moq;
using Piipan.Metrics.Api;
using Piipan.Metrics.Func.Api.Extensions;
using Xunit;

namespace Piipan.Metrics.Func.Api.Tests
{
    public class GetParticipantUploadsTests
    {
        [Fact]
        public async Task Run_Success()
        {
            // Arrange
            var uploadedAt = DateTime.Now;

            var context = new DefaultHttpContext();
            var request = context.Request;
            request.QueryString = new QueryString("");

            Assert.Equal(0, request.Query.ParseInt("perPage", 0));

            var logger = new Mock<ILogger>();

            var expectedResponse = new GetParticipantUploadsResponse
            {
                Data = new List<ParticipantUpload>
                {
                    new ParticipantUpload
                    {
                        State = "ea",
                        UploadedAt = uploadedAt
                    }
                },
                Meta = new Meta
                {
                    PerPage = 1,
                    Total = 1
                }
            };

            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ReturnsAsync(expectedResponse);

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act
            var result = (await function.Run(request, logger.Object)) as JsonResult;
            var response = result.Value as GetParticipantUploadsResponse;

            // Assert
            Assert.NotNull(response);
            Assert.Equal(response.Meta, expectedResponse.Meta);
            Assert.Equal(response.Data, expectedResponse.Data);
            logger.Verify(x => x.Log(
                It.Is<LogLevel>(l => l == LogLevel.Information),
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("Executing request from user")),
                It.IsAny<Exception>(),
                It.Is<Func<It.IsAnyType, Exception, string>>((v, t) => true)
            ));
        }

        [Fact]
        public async Task Run_UploadApiThrows()
        {
            // Arrange
            var context = new DefaultHttpContext();
            var request = context.Request;

            var logger = new Mock<ILogger>();

            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ThrowsAsync(new Exception("upload api broke"));

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act / Assert
            await Assert.ThrowsAsync<Exception>(() => function.Run(request, logger.Object));
            logger.Verify(x => x.Log(
                It.Is<LogLevel>(l => l == LogLevel.Error),
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString() == "upload api broke"),
                It.IsAny<Exception>(),
                It.Is<Func<It.IsAnyType, Exception, string>>((v, t) => true)
            ));
        }
        [Theory]
        [InlineData("1/1/1888")] // start Date
        [InlineData("1/1/2051")] // start Date
        public async Task Run_UploadApiThrowsValidation_StartDate(string startDate)
        {
            // Arrange
            var uploadedAt = DateTime.Now;

            var context = new DefaultHttpContext();
            var request = context.Request;
            request.QueryString = new QueryString("");
            request.QueryString = request.QueryString.Add("StartDate", startDate);

            Assert.Equal(0, request.Query.ParseInt("perPage", 0));

            var logger = new Mock<ILogger>();

            var expectedResponse = new GetParticipantUploadsResponse
            {
                Data = new List<ParticipantUpload>
                {
                    new ParticipantUpload
                    {
                        State = "ea",
                        UploadedAt = uploadedAt
                    }
                },
                Meta = new Meta
                {
                    PerPage = 1,
                    Total = 1
                }
            };
            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ReturnsAsync(expectedResponse);

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act / Assert
            var result = (await function.Run(request, logger.Object)) as JsonResult;
            var response = result.Value as string;

            // Assert

            Assert.True(response.Contains("Start Date must have a year between 1900 and 2050"));
        }
        [Theory]
        [InlineData("1/1/1888")] // start Date
        [InlineData("1/1/2051")] // start Date
        public async Task Run_UploadApiThrowsValidation_EndDate(string endDate)
        {
            // Arrange
            var uploadedAt = DateTime.Now;

            var context = new DefaultHttpContext();
            var request = context.Request;
            request.QueryString = new QueryString("");
            request.QueryString = request.QueryString.Add("EndDate", endDate);

            Assert.Equal(0, request.Query.ParseInt("perPage", 0));

            var logger = new Mock<ILogger>();

            var expectedResponse = new GetParticipantUploadsResponse
            {
                Data = new List<ParticipantUpload>
                {
                    new ParticipantUpload
                    {
                        State = "ea",
                        UploadedAt = uploadedAt
                    }
                },
                Meta = new Meta
                {
                    PerPage = 1,
                    Total = 1
                }
            };
            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ReturnsAsync(expectedResponse);

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act / Assert
            var result = (await function.Run(request, logger.Object)) as JsonResult;
            var response = result.Value as string;

            // Assert

            Assert.True(response.Contains("End Date must have a year between 1900 and 2050"));
        }
        [Fact]
        public async Task Run_UploadApiThrowsValidation_EndDate_LessThan_StartDate()
        {
            // Arrange
            var uploadedAt = DateTime.Now;

            var context = new DefaultHttpContext();
            var request = context.Request;
            request.QueryString = new QueryString("");
            request.QueryString = request.QueryString.Add("EndDate", "1/1/2023");
            request.QueryString = request.QueryString.Add("StartDate", "1/15/2023");


            Assert.Equal(0, request.Query.ParseInt("perPage", 0));

            var logger = new Mock<ILogger>();

            var expectedResponse = new GetParticipantUploadsResponse
            {
                Data = new List<ParticipantUpload>
                {
                    new ParticipantUpload
                    {
                        State = "ea",
                        UploadedAt = uploadedAt
                    }
                },
                Meta = new Meta
                {
                    PerPage = 1,
                    Total = 1
                }
            };
            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ReturnsAsync(expectedResponse);

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act / Assert
            var result = (await function.Run(request, logger.Object)) as JsonResult;
            var response = result.Value as string;

            // Assert

            Assert.True(response.Contains("End Date cannot be before the Start Date"));
        }
        [Fact]
        public async Task Run_Success_With_EndDate_StartDate()
        {
            // Arrange
            var uploadedAt = DateTime.Now;

            var context = new DefaultHttpContext();
            var request = context.Request;
            request.QueryString = new QueryString("");
            request.QueryString = request.QueryString.Add("StartDate", "1/1/2022");
            request.QueryString = request.QueryString.Add("EndDate", "1/15/2023");


            Assert.Equal(0, request.Query.ParseInt("perPage", 0));

            var logger = new Mock<ILogger>();

            var expectedResponse = new GetParticipantUploadsResponse
            {
                Data = new List<ParticipantUpload>
                {
                    new ParticipantUpload
                    {
                        State = "ea",
                        UploadedAt = uploadedAt
                    }
                },
                Meta = new Meta
                {
                    PerPage = 1,
                    Total = 1
                }
            };
            var uploadApi = new Mock<IParticipantUploadReaderApi>();
            uploadApi
                .Setup(m => m.GetUploads(It.IsAny<ParticipantUploadRequestFilter>()))
                .ReturnsAsync(expectedResponse);

            var function = new GetParticipantUploads(uploadApi.Object);

            // Act / Assert
            var result = (await function.Run(request, logger.Object)) as JsonResult;
            var response = result.Value as GetParticipantUploadsResponse;
            // Assert

            Assert.NotNull(response);
            Assert.Equal(response.Meta, expectedResponse.Meta);
            Assert.Equal(response.Data, expectedResponse.Data);
            logger.Verify(x => x.Log(
                It.Is<LogLevel>(l => l == LogLevel.Information),
                It.IsAny<EventId>(),
                It.Is<It.IsAnyType>((v, t) => v.ToString().Contains("Executing request from user")),
                It.IsAny<Exception>(),
                It.Is<Func<It.IsAnyType, Exception, string>>((v, t) => true)
            ));

        }
    }
}
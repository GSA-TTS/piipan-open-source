...
{
    /// <summary>
    /// A collection of tests that verifies the changes made to MultipleDatabaseManager, including how it performs queries and creates connection strings
    /// over multiple threads.
    /// </summary>
    [Collection("Piipan.Shared.DatabaseManager")]
    public class MultipleDatabaseManagerTests
    {
        ...
        private string[] MockDatabases = new string[] { "ea", "eb" };
        ...
        private async Task<int> PerformDummyQuery(MultipleDatabaseManager<MockType> databaseManager, string database = "ea")
        {
            return await databaseManager.PerformQuery(conn => Task.FromResult(1), database);
        }
        private async Task<int> PerformDummyQueryForPostgresExceptionRetry(MultipleDatabaseManager<MockType> databaseManager, string database = "ea", string errorCode="")
        {
            int count = 0;
       ...
       [Fact]
        public void CreateManager_NoDatabaseConnectionString()
        {
            ...
            Assert.Throws<ArgumentException>(() =>
                new MultipleDatabaseManager<MockType>(
                    String.Empty, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object)
            );
        }
        ...
        public void CreateManager_MalformedDatabaseConnectionString()
        {
            ...
            // Act / Assert
            Assert.Throws<ArgumentException>(() => new MultipleDatabaseManager<MockType>(
                    malformedString, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object)
            );
        }
        ...
        public async void CreateManagerAndPerformQuery_DefaultsToCommercialCloud()
        {
            ...
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, azureTokenProvider.Object);
            ...
            azureTokenProvider.Verify(m =>
                m.GetAccessTokenAsync(MultipleDatabaseManager<MockType>.CommercialId, null, default(CancellationToken)), Times.Once);
        }
        ...
        public async void CreateManagerAndPerformQuery_UsesGovtCloudWhenSet()
        {
            ...
            try
            {
                Environment.SetEnvironmentVariable(
                    MultipleDatabaseManager<MockType>.CloudName,
                    MultipleDatabaseManager<MockType>.GovernmentCloud
                );
                ...
                // Assert
                azureTokenProvider.Verify(m =>
                    m.GetAccessTokenAsync(MultipleDatabaseManager<MockType>.GovermentId, null, default(CancellationToken)), Times.Once);
        ...
        public async void PerformQuery_DoesNotFetchToken_WhenNotNeeded()
        {
            ...
            var azureTokenProvider = DefaultMocks.MockAzureServiceTokenProvider();
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionStringWithoutNeedingToken, MockDatabases, azureTokenProvider.Object);
        ...
        [Fact]
        public async Task PerformQuery_UsesDatabaseOverride_MixedCase()
        {
            // Arrange
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object);

            var usedConnectionString = "";
            ...
            // Assert
            Assert.Contains($"Database=eb", usedConnectionString);
        }

        [Fact]
        public async Task PerformQuery_OnNonExistingDatabase_ThrowsError()
        {
            // Arrange
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object);

            // Act / Assert. ec database was not registered to the MultipleDatabaseManager so an error should be thrown.
            await Assert.ThrowsAsync<ArgumentException>(() => PerformDummyQuery(databaseManager, "ec"));
        }
        
        [Fact]
        public async Task PerformQuery_OnMutlipleDatabases_SimultaneouslyWorksFine()
        {
            // Arrange
            ClearEnvironmentVariables();
            var mockTokenProvider = DefaultMocks.MockAzureServiceTokenProvider();
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, mockTokenProvider.Object);
            int numberOfTasksCompleted = 0;
            int numberOfTasksToDo = 2000;
            int numberOfErrors = 0;

            // Act - Kick off a bunch of tasks with half to each database to make sure the database manager can handle multiple threads at once
        ...
        [Theory]
        public async Task PerformQuery_For_ErrorCodesInPolicyForRetry_PostgresException(string errorCode)
        {
            // Arrange
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object);
            ...
            // Act / Assert.   MultipleDatabaseManager  throw an error which is part of the Polly retry policy.
        ...
        [Theory]
        public async Task PerformQuery_For_ErrorCodesNotInPolicyForRetry_PostgresException(string errorCode)
        {
            // Arrange
            var databaseManager = new MultipleDatabaseManager<MockType>(
                    ConnectionString, MockDatabases, DefaultMocks.MockAzureServiceTokenProvider().Object);
